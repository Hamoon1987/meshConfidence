...
def run_evaluation(model, dataset_name, dataset,
    ...
    for step, batch in enumerate(tqdm(data_loader, desc='Eval', total=len(data_loader))):
        ...
        _, sp_gt_max_ind = torch.topk(sp_gt, 3)
        sp_op_max_ind_sorted, _ = torch.sort(sp_op_max_ind, dim=1)
        sp_gt_max_ind_sorted, _ = torch.sort(sp_gt_max_ind, dim=1)
        print(sp_op_max_ind_sorted)
        print(sp_gt_max_ind_sorted)
        eval = torch.eq(sp_op_max_ind_sorted, sp_gt_max_ind_sorted)
        eval = eval.cpu().numpy()
        print(eval)
