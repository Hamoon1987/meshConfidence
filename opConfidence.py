"""
Investigate the correlation of OpenPose confidence and SPIN accuracy
Example usage:
```
python3 opConfidence.py --checkpoint=data/model_checkpoint.pt --dataset=3dpw --log_freq=20
```
...
def run_evaluation(model, dataset_name, dataset, result_file,
    ...
    for step, batch in enumerate(tqdm(data_loader, desc='Eval', total=len(data_loader))):
        ...
        if eval_pose:
            ...
            # OpenPose Confidence
            error_ = candidate_sorted_t[:, 6].cpu().numpy()
            # error_ = torch.sqrt(((pred_keypoints_2d_n[:, 6, :] - candidate_sorted_t[:, 6, :]) ** 2).sum(dim=-1)).cpu().numpy()
