...
Example usage:
```
python3 sp_op/3dpw_test.py --checkpoint=/SPINH/data/model_checkpoint.pt --dataset=3dpw --log_freq=20
```
...
def run_evaluation(model, joint_index, dataset_name, dataset, result_file,
    ...
    joint_mapper_gt_label_3d = [8, 5, 2, 1, 4, 7, 21, 19, 17, 16, 18, 20, 12, 15, 6] # 6 is spine
    ...
    smpl_joint_map_gt = [11, 10, 9, 12, 13, 14, 4, 3, 2, 5, 6, 7, 37, 42, 41]
    ...
    smpl_pred_keypoints_2d_n = torch.sub(smpl_pred_keypoints_2d, (constants.IMG_RES/2))
    smpl_pred_keypoints_2d_n = torch.div(smpl_pred_keypoints_2d_n, (constants.IMG_RES/2))
    ...
    error = torch.sqrt(((smpl_pred_keypoints_2d_n[:, joint_index, :] - gt_label_2d_n[:, joint_index, :]) ** 2).sum(dim=-1)).cpu().numpy()
    mpjpe[step * batch_size:step * batch_size + curr_batch_size] = error

    # SPIN - OpenPose (sp_op)
    error_ = torch.sqrt(((smpl_pred_keypoints_2d_n[:, joint_index, :] - candidate_sorted_t_n[:, joint_index, :]) ** 2).sum(dim=-1)).cpu().numpy()
    ...
    # for i in range(15):
    #     cv2.circle(image_test, (int(smpl_pred_keypoints_2d_gt[i][0]), int(smpl_pred_keypoints_2d_gt[i][1])), 2, color = (255, 0, 0), thickness=-1)
    cv2.imwrite(f'sp_op/3dpw_test.png', image_test)
