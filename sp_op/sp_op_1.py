...
Example usage:
```
python3 sp_op/sp_op_01.py --checkpoint=/SPINH/data/model_checkpoint.pt --dataset=mpi-inf-3dhp --log_freq=20
```
...
def get_occluded_imgs(batch, args, batch_idx, joint_index):
    ...
    if batch_idx % (10*log_freq) == (10*log_freq) - 1:
        ...
        occ_images_denormalized = denormalize(occ_images)
        img = occ_images_denormalized[0]
        cv2.imwrite(os.path.join(out_path, f'occluded_{joint_idx}_{(batch_idx*batch_size):05d}.jpg'), img)
...
def run_evaluation(model, joint_index, dataset_name, dataset, result_file,
    ...
    op_mpjpe = np.zeros(len(dataset))
    mpjpe_2d = np.zeros(len(dataset))
    ...
    for step, batch in enumerate(tqdm(data_loader, desc='Eval', total=len(data_loader))):
        ...
        if occ_joint:
            batch['img'] = get_occluded_imgs(batch, args, step, joint_index)
        images = batch['img'].to(device)
        ...
        # Get 14 3D ground truth joints
        gt_keypoints_3d = batch['pose_3d'].cuda()
        gt_keypoints_3d = gt_keypoints_3d[:, joint_mapper_gt, :-1]
        # Get 14 2D ground truth joints
        gt_keypoints_2d_n = batch["keypoints"]
        gt_keypoints_2d_n = gt_keypoints_2d_n[gt_keypoints_2d_n[:,:,2] == 1]
        gt_keypoints_2d_n = torch.reshape(gt_keypoints_2d_n, (curr_batch_size, -1, 3))
        gt_keypoints_2d_n = gt_keypoints_2d_n[:,:,:2].to(device)
        ...
        # Absolute error 2D SPIN (MPJPE_2d)
        error_2d = torch.sqrt(((pred_keypoints_2d_n[:, joint_index, :] - gt_keypoints_2d_n[:, joint_index, :]) ** 2).sum(dim=-1)).cpu().numpy()
        mpjpe_2d[step * batch_size:step * batch_size + curr_batch_size] = error_2d
        ...
        # OpenPose Error
        op_error = torch.sqrt(((gt_keypoints_2d_n[:, joint_index, :] - candidate_sorted_t_n[:, joint_index, :]) ** 2).sum(dim=-1)).cpu().numpy()
        op_mpjpe[step * batch_size:step * batch_size + curr_batch_size] = op_error
    ...
    if occ_joint:
        ...
        np.save(f'sp_op/occ_mpjpe_{joint_index}.npy', mpjpe) # save
        np.save(f'sp_op/occ_conf_{joint_index}.npy', op_conf) # save
    else:
        np.save(f'mpi_sp_op/sp_op_{joint_index}.npy', sp_op) # save
        np.save(f'mpi_sp_op/mpjpe_{joint_index}.npy', mpjpe) # save
        np.save(f'mpi_sp_op/conf_{joint_index}.npy', op_conf) # save
        np.save(f'mpi_sp_op/op_mpjpe_{joint_index}.npy', op_mpjpe) # save
        np.save(f'mpi_sp_op/mpjpe_2d_{joint_index}.npy', mpjpe_2d) # save
