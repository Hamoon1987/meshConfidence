...
def run_evaluation(model, joint_index, dataset_name, dataset, result_file,
    ...
    for step, batch in enumerate(tqdm(data_loader, desc='Eval', total=len(data_loader))):
        ...
        # Replacing hips, head and neck whith the SMPL regressed hips  
        pred_keypoints_3d_[:, 2, :] = pred_joints[:, 9, :]
        pred_keypoints_3d_[:, 3, :] = pred_joints[:, 12, :]
        pred_keypoints_3d_[:, 13, :] = pred_joints[:, 15, :]
        pred_keypoints_3d_[:, 12, :] = pred_joints[:, 40, :]
        ...
        # Get the GT based on SMPL joints regressor and the mesh
        ...
        gt_joints_smpl[gender==1, :, :] = gt_joints_smpl_female[gender==1, :, :]

        # Replace the Head with SMPL regressed joints
        gt_keypoints_3d_[:, 13, :] = gt_joints_smpl[:, 15, :]
        # gt_keypoints_3d_[:, 12, :] = gt_joints_smpl[:, 40, :]
        ...
        gt_keypoints_2d = perspective_projection(gt_keypoints_3d_r,
                                    rotation=torch.eye(3, device=device).unsqueeze(0).expand(curr_batch_size, -1, -1),
                                    translation=camera_translation,
                                    focal_length=focal_length,
                                    camera_center=camera_center)
        ...
        if (joint_index ==13):
            error = torch.sqrt(((pred_keypoints_2d_n[:, joint_index, :] - gt_keypoints_2d_n[:, joint_index, :]) ** 2).sum(dim=-1)).cpu().numpy()
        else:
