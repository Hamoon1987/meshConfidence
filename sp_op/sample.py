...
parser.add_argument('--dataset', default='h36m-p2', choices=['h36m-p1', 'h36m-p2', '3doh', '3dpw', 'mpi-inf-3dhp'], help='Choose evaluation dataset')
...
def get_occluded_imgs(batch, args, new_p):
    # Get the image batch choose a random joint  and occlude it.
    # positions and projects them.
    occ_size = args.occ_size
    occ_pixel = args.pixel
    joint_idx = random.randint(0,12)
...
def run_evaluation(model, dataset_name, dataset,
    ...
    # print("sp_gt", error)
    error = torch.sqrt(((smpl_pred_keypoints_2d_gt - gt_keypoints_2d) ** 2).sum(dim=-1)).cpu().numpy()
    sp_gt[step * batch_size:step * batch_size + curr_batch_size] = error
    # print("sp_gt_real", error)
    # SPIN - OpenPose (sp_op)
    ...
    # print("sp_op", error_)
    error_ = torch.sqrt(((smpl_pred_keypoints_2d_op - candidate_sorted_t) ** 2).sum(dim=-1)).cpu().numpy()
    sp_op[step * batch_size:step * batch_size + curr_batch_size] = error_
    # print("sp_op_real", error_)
    # OpenPose Confidence
    op_confidence_joint = op_confidence_t.cpu().numpy()
    # print("conf", op_confidence_joint[0,6])
    ...
    print(back.shape)
    for i in range(14):
        # cv2.circle(back, (int(gt_keypoints_2d[i][0]), int(gt_keypoints_2d[i][1])), 3, color = (0,0,0), thickness=-1)
        # cv2.circle(back, (int(candidate_sorted_t[i][0]), int(candidate_sorted_t[i][1])), 3, color = (0, 0, 255), thickness=-1)
        cv2.circle(back, (int(smpl_pred_keypoints_2d_gt[i][0]), int(smpl_pred_keypoints_2d_gt[i][1])), 3, color = (0.12,0.12,0.12), thickness=-1)
    ...
    cv2.imwrite(f'sp_op/test.png', 255 * back)
