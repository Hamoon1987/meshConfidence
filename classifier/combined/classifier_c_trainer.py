...
from classifier_c_dataloader import fetch_dataloader
import torch
from classifier_config import args
import matplotlib.pyplot as plt

net = Model(14, 28, 56, 28, 15)
device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
print(device)
net = net.to(device)
criterion = torch.nn.CrossEntropyLoss()
optimizer = torch.optim.Adam(net.parameters(), lr = 0.0001)
train_dataloader, val_dataloader = fetch_dataloader(args.all_train_path, "train")
epochs = args.epochs
...
# Save the model
torch.save(net, 'classifier/classifier_c.pt')
